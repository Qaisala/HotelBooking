    ...
    <form className="mt-4 p-4">
      ...
      <div className="w-full flex sm:flex-col sm:gap-4 mt-4">
        ...
        <div>
          ...
          type="number"
          placeholder="0"
          className="border border-gray-300 mt-2 rounded w-25"
            value={inputData.pricePerNight} 
            onChange={(e) =>
              setInputData({ ...inputData, pricePerNight: e.target.value })
            }
          />
        </div>
      </div>  
      ...
      <div className="flex flex-col flex-wrap text-gray-400 mt-2">
        {Object.keys(inputData.amenities).map((amenity,i) => (
          <div key={i}>
            <input
              type="checkbox"
              id={`amenities${i+1}`}
              ...
              onChange={() =>
                setInputData({
                  ...
                  amenities: {
                    ...inputData.amenities, 
                    [amenity]: !inputData.amenities[amenity] }})} />
            <label htmlFor={`amenities${i+1}`} className="ml-2">{amenity}</label>
          </div>
        ))}
      </div>
